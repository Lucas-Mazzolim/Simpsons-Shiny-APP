...
Neste trabalho, será apresentada uma análise dos episódios das temporadas 21 a 28, exibidos entre os anos de 2010 a 2016, considerando dados de audiência, avaliação do público e caracaterísticas dos diálogos entre os personagens. Pretende-se avaliar as tendências de variação da audiência e da avaliação popular dos episódios ao longo do tempo, e também a existência de correlações entre essas duas variáveis. Serão verificadas, também, tendências de variação no volume de palavras faladas pelos personagens, possíveis correlações entre este volume e a avaliação popular, além de diferenças relacionadas ao gênero dos personagens.
...
ggplot(views_by_season, aes(season, mean_viewers)) +
  ...
  labs(
    ...
    x = "Temporada",
    y = "Audiência média (milhões de espectadores)"
  ) +
  theme_bw() +
  theme(
...
ggplot(corr_by_season_votes, aes(x = season, y = corr)) +
  geom_line(linewidth = 0.65, color = "darkred") +
  geom_point(size = 2, shape = 21, color = "#333234", fill = "#EB8495") +
  labs(
    title = "Correlação entre audiência e notas IMDb por temporada",
    x = "Temporada",
...
<p>A análise dos diálogos fixou-se, principalmente, na quantidade de palavras ou frases faladas pelos personagens. Inicialmente, uma análise do número de falas dos personagens resultou no gráfico seguinte.</p><br>
...
<p>Na sequência das análises, avaliou-se a variação entre as temporadas do volume de falas dos personagens, focando apenas nos membros da família. A partir desta análise, decidiu-se investigar uma possível influência dos volumes de falas sobre as notas do IMDb vistas anteriormente. Os resultados são mostrados nos gráficos a seguir.</p><br>
